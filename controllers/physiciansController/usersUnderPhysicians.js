...
export const fetchUsersBookedPhysician = async (req, res) => {
  ...
  try {
    const usersUnderPhysician = await usersUnderPhysicians.findOne({
      doctorId: physicianId,
    });
    if (usersUnderPhysician?.userList) {
      res.json(Object.assign(usersUnderPhysician?.userList));
    } else {
      res.json(usersUnderPhysician);
    }
  } catch (error) {
